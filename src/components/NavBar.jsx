...
// useEffect -> runs code AFTER the components appear on the screen
...
// motion will be underlines with a red squiggly, but just ignore it because eslint pertains to javascript and doesn't realize that we are using jsx specific items
...
export default function NavBar() {
    ...
    useEffect(() => {
        // Runs on every scroll; true once we're more than 2-px down the page
        const handleScroll = () => setScrolled(window.scrollY > 20);

        // { passive: true } promises we won't block scrolling, which lets the browser keep the page smooth.
        window.addEventListener("scroll", handleScroll, { passive: true });

        // The returned function is "cleanup" = so React runs it when the component is removed, so we don't leave a dead listener behind.
        return () => window.removeEventListener("scroll", handleScroll);
    ...
    return (
        <motion.header
            ...
            // initial = where it stars (80px above, invisible), animate = where it ends
